...
_CLOZE_JSON_EXAMPLE = """{
  ...
  "explanation": "逐空解析（中文）：说明正确项依据及每个干扰项的排除理由"
}"""
...
_READING_JSON_EXAMPLE = """{
  ...
  "answers": ["A", "C"],
  "explanations": ["逐题解析（中文）：指出答案在文中的依据与各干扰项排除理由"]
}"""
...
【语言要求】
- 文章正文须为英文（符合英语考试语体）。
- title（标题）与 explanation（逐空解析）必须全部使用中文撰写，便于用户理解，不得出现英文解析。
...
- 以四选一选择题（A/B/C/D，仅一个正确）为主，共 {total} 道题；其中可包含不超过 1-2 道主观问答题（开放式，不要给 A-D 选项，答案给出参考要点）
- 分值说明：{spec['scoring']}
...
- 主观问答题须能从原文归纳作答，答案给参考要点即可，不要给 A-D 选项

【语言要求】
- 文章正文须为英文（符合英语考试语体）。
- questions（题干）可用中文表述；explanations（逐题解析）、主观题答案参考要点必须全部使用中文，不得出现英文解析。
