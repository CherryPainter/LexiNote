...
class WordManager:
    ...
    def __init__(self, statistics_manager: Optional[StatisticsManager] = None):
        ...
        self._init_thread = threading.Thread(
            target=self._init_background, daemon=True)
    ...
    def _check_throttle_limit(self) -> bool:
        ...
        with self._throttle_lock:
            ...
            # 移除过期的调用记录
            while self._recent_ai_calls and current_time - \
                    self._recent_ai_calls[0] > 60:  # 超过1分钟的记录
            ...
            # 检查两次调用之间的最小间隔
            if self._recent_ai_calls and current_time - \
                    self._recent_ai_calls[-1] < self._min_interval_ms / 1000:
    ...
    def set_throttle_limits(
        ...
        with self._throttle_lock:
            ...
            self._max_calls_per_minute = max(
                1, max_calls_per_minute)  # 至少1次/分钟
            log_info(
                f"已设置AI节流参数: 最小间隔{
                    self._min_interval_ms}ms, 最大频率{
                    self._max_calls_per_minute}次/分钟")
        ...
        try:
            ...
            if 'name' not in data:
                ...
                data['name'] = os.path.splitext(
                    os.path.basename(json_file_path))[0]
    ...
    def get_words_missing_details(self, limit=10):
        ...
        AND (
            ...
            example_translation IS NULL OR example_translation = '' OR
        ...
        try:
            return self.db_manager.execute_read(
                sql, (self.active_word_set_id, limit))
        except Exception as e:
    ...
    def _format_translation(
            self, translation: Union[str, List[Dict[str, Any]]]) -> str:
        """格式化翻译结果为字符串
        ...
        try:
            if isinstance(
                    translation, str) and (
                    translation.startswith('[') or translation.startswith('{')):
    ...
    def update_word_familiarity(self, word: str, value: float, delta: bool = False):
        """更新单词熟悉度（兼容旧接口，实际使用proficiency字段）

        Args:
            word: 单词
            value: 熟悉度值（0.0-1.0）或变化量
            delta: 是否将value视为变化量（True）或绝对值（False）
        """
        try:
            if delta:
                # 视为变化量，调用更新熟练度方法
                is_correct = value > 0  # 正增量表示正确，负增量表示错误
                ...
                if result:
                    ...
                    with self._cache_lock:
                        if word in self.word_familiarity:
                            self.word_familiarity[word] = float(value)

                    if update_result[0]:
                        log_info(f"更新单词熟悉度成功: {word} -> {value}")
                    else:
    ...
    def get_word_familiarity(
            self, word: str = None) -> Union[float, Dict[str, float]]:
        """获取单词熟悉度
    ...
    def _load_word_familiarity(self):
        ...
        try:
            ...
            with self._cache_lock:
                self.word_familiarity = {row['word']: (row.get('proficiency') if row.get(
                    'proficiency') is not None else 0.0) for row in rows}
    ...
    def _load_active_word_set(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT value FROM settings WHERE key = 'active_word_set'")
    ...
    def get_random_word(
        ...
        try:
            ...
            if exclude_words:
                available_words = [
                    word for word in all_words if word not in exclude_words]
            else:
    ...
    def get_weighted_random_word(
        ...
        try:
            ...
            if exclude_words:
                words = [
                    word for word in words if word['word'] not in exclude_words]
    ...
    def update_word_weight(
        ...
        try:
            ...
            log_info(
                f"更新单词权重: {word}, 熟练度从 {current_proficiency} 调整为 {new_proficiency}")
    ...
    def get_today_learned_words(self) -> List[str]:
        ...
        try:
            ...
            try:
                all_words = self.db_manager.get_all_words()
                return {
                    word: self.word_familiarity.get(
                        word.lower(),
                        0.0) for word in all_words}
            except Exception as e:
    ...
    def get_and_save_word_attributes(self,
                                     word: str,
        ...
        try:
            ...
            valid_attributes = [
                ...
                'tag']
            attributes = [
                attr for attr in attributes if attr in valid_attributes]
            ...
            def fetch_missing_attributes():
                try:
                    ...
                    # 请求AI获取缺失的属性
                    # 构建提示词请求所有缺失的属性
                    prompt = f"请为单词 '{word}' 提供以下信息：{
                        ', '.join(missing_attributes)}"
                    if 'example' in missing_attributes:
                        prompt += "，例句需要包含英文句子和中文翻译"

                    _ = self.ai_manager._ask_sync(prompt)
                    ...
                    if 'phonetic' in missing_attributes:
                        # 获取音标
                        try:
                            phonetic_response = self.ai_manager._ask_sync(
                                f"请提供单词 '{word}' 的标准音标，仅返回音标部分")
                    ...
                    if 'example' in missing_attributes:
                        ...
                        try:
                            ...
                            if example and "AI功能暂不可用" not in example and "生成例句失败" not in example:
                                # 正确拆分例句和翻译
                                if "|" in example:
                                    example_en, example_zh = example.split(
                                        "|", 1)
                    ...
                    if 'meaning_en' in missing_attributes:
                        # 获取英文释义
                        try:
                            meaning_en_response = self.ai_manager._ask_sync(
                                f"请提供单词 '{word}' 的英文释义，仅返回英文")
                    ...
                    if 'tag' in missing_attributes:
                        # 获取标签
                        try:
                            tag_response = self.ai_manager._ask_sync(
                                f"请为单词 '{word}' 提供合适的标签，用逗号分隔，如：名词,动词")
                    ...
                    def save_attributes_to_database():
                        try:
                            ...
                            if update_data:
                                success, msg = self.update_word(
                                    word_obj['id'], **update_data)
                                if success:
                                    log_info(
                                        f"已保存单词属性到数据库: {word}, 属性: {
                                            ', '.join(
                                                update_data.keys())}")
                                else:
                    ...
                    save_thread = threading.Thread(
                        target=save_attributes_to_database, daemon=True)
                    save_thread.start()
            ...
            if async_mode:
                thread = threading.Thread(
                    target=fetch_missing_attributes, daemon=True)
                thread.start()
                return None
            else:
                # 同步模式：直接执行并返回结果
    ...
    def get_word_example(
        ...
        def example_callback(attributes):
            if callback:
                try:
                    ...
                    example_translation = attributes.get(
                        'example_translation', '')
                    if example and example_translation:
                        formatted_example = f"🌍 {example}\n📝 {example_translation}"
                    elif example:
        ...
        attributes = self.get_and_save_word_attributes(
            word,
            [
                'example',
                'example_translation'],
    ...
    def migrate_old_translations(self):
        ...
        try:
            ...
            # 获取所有单词
            all_words = self.db_manager.execute_read(
            ...
            for word_data in all_words:
                ...
                if isinstance(translation,
                              str) and translation.startswith('['):
                    try:
                        parsed = json.loads(translation)
                        if isinstance(
                            parsed,
                ...
                if new_translation:
                    # 更新数据库
                    new_translation_json = json.dumps(
                        new_translation, ensure_ascii=False)
                    success, msg = self.update_word(
                        word_id, translation=new_translation_json)
                    if success:
                        migrated_count += 1
    ...
    def check_translation(
            ...
            translation_mode: str = 'ai_first') -> bool:
        """检查用户翻译是否正确
        ...
        try:
                ...
                for item in correct_translation:
                    if isinstance(item, dict) and 'meaning_zh' in item:
                        ...
                        if isinstance(meanings, list):
                            candidates.extend(
                                [normalize(m) for m in meanings if m and m.strip()])
                ...
                try:
                    ...
                    if correct_translation.startswith('['):
                        ...
                        if isinstance(parsed, list):
                            for item in parsed:
                                if isinstance(
                                        item, dict) and 'meaning_zh' in item:
                                    meanings = item['meaning_zh']
                                    if isinstance(meanings, list):
                                        candidates.extend(
                                            [normalize(m) for m in meanings if m and m.strip()])
                            ...
                            if candidates:
                                ...
                                raise json.JSONDecodeError(
                                    "Invalid format", correct_translation, 0)
                        else:
                            # 解析结果不是列表，继续按字符串处理
                            raise json.JSONDecodeError(
                                "Not a list", correct_translation, 0)
                    else:
                        # 不是JSON格式，按字符串处理
                        raise json.JSONDecodeError(
                            "Not JSON", correct_translation, 0)
                except json.JSONDecodeError:
                    # 按旧格式字符串处理
                    candidates_raw = re.split(r"[;,；、/|]", correct_translation)
                    candidates = [
                        normalize(c) for c in candidates_raw if c and c.strip()]
            ...
            if translation_mode == 'local_first' or translation_mode == 'local_only':
                ...
                for cand in candidates:
                    ...
                    if user_normalized and (
                            user_normalized in cand or cand in user_normalized):
                        is_correct = True
                        break
                ...
                if not is_correct and translation_mode == 'local_first' and self.ai_available and self.ai_manager:
                    try:
                        # 使用格式化后的翻译进行AI评估
                        formatted_translation = self._format_translation(
                            correct_translation)
                        eval_result = self.ai_manager.evaluate_sync(
                            formatted_translation, user_translation)
                        if isinstance(eval_result, dict):
                            ai_is_correct = bool(eval_result.get('is_correct'))
                            similarity = float(eval_result.get('similarity', 0)) if eval_result.get(
                                'similarity') is not None else 0.0
                            # Accept when AI says correct, or similarity is
                            # high (>=0.8)
            ...
            if update_stats:
                if is_correct:
                    ...
                    log_info(
                        f"翻译正确: {word} -> {user_translation}, mode={translation_mode}")
                else:
                    # 翻译错误，更新熟练度
                    self.update_word_proficiency(word_lower, False)
                    self.update_word_weight(word_lower, False, 0)
                    log_info(
                        f"翻译错误: {word} -> 用户输入: {user_translation}, 正确翻译: {correct_translation}, mode={translation_mode}")
