...
class TranslationPage:
    ...
    def _check_translation(self):
        ...
        # 检查翻译
        if self.is_english_to_chinese:
            is_correct = self.word_manager.check_translation(self.current_translation, user_input, True)
        else:
            is_correct = self.word_manager.check_translation(self.current_word, user_input, False)
        ...
        if is_correct:
            ...
            log_info(f"翻译正确: {self.current_word} -> {self.current_translation}")
        else:
            if self.is_english_to_chinese:
                self.result_var.set(f"✗ 错误！正确翻译: {self.current_translation}")
            else:
                self.result_var.set(f"✗ 错误！正确翻译: {self.current_word}")
    ...
    def _skip_translation(self):
        ...
        if self.is_english_to_chinese:
            self.result_var.set(f"⏭️ 已跳过: {self.current_word} -> {self.current_translation}")
        else:
            self.result_var.set(f"⏭️ 已跳过: {self.current_translation} -> {self.current_word}")
        self.result_label.config(fg='#FF9800')
        log_info(f"跳过翻译: {self.current_word}")
        
        # 显示下一个
        self.main_frame.after(1000, self._next_translation)
