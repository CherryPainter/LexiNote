...
class CacheManager:
        ...
        with self._lock:
            if not hasattr(self, '_initialized'):
                ...
                self.history_file = os.path.join(
                    self.cache_root, 'ai_history.json')
    ...
    def get_text_cache(self, key: str) -> Optional[Dict[str, Any]]:
        ...
        cache_file = os.path.join(
                    self.text_cache_dir,
                    f"{self._generate_cache_key(key)}.json")
    ...
    def set_text_cache(
            self, key: str, data: Dict[str, Any], ttl: int = 30 * 24 * 3600):
        """设置文本缓存
        ...
        try:
            ...
            cache_file = os.path.join(
                self.text_cache_dir, f"{
                    self._generate_cache_key(key)}.json")
    ...
    def get_tts_cache(
        ...
        cache_file = os.path.join(
            self.tts_cache_dir,
            f"{self._generate_cache_key(key)}.mp3")
        ...
        try:
            ...
            with self._cache_lock:
                ...
                if os.path.exists(self.history_file):
                    try:
                        with open(
                            self.history_file, 'r', encoding='utf-8'
                        ) as f:
                            history = json.load(f)
    ...
    def clean_old_cache(self, days: int = 30):
        ...
        try:
            ...
            # 清理文本缓存
            text_removed = self._clean_directory(
                self.text_cache_dir, cutoff_time)

            # 清理语音缓存
            tts_removed = self._clean_directory(
                self.tts_cache_dir, cutoff_time)
    ...
    def _clean_memory_cache(self):
        ...
        with self._cache_lock:
            ...
            if len(self._memory_cache) > 500:
                ...
                self._memory_cache = {k: v for k, v in
    ...
    def get_cache_stats(self) -> Dict[str, int]:
        ...
        try:
            # 文本缓存数量
            text_files = [f for f in os.listdir(self.text_cache_dir)
                          if f.endswith('.json')]
            text_count = len(text_files)

            # 语音缓存数量
            tts_files = [f for f in os.listdir(self.tts_cache_dir)
                         if f.endswith('.mp3')]
            ...
            return {
                ...
                'total_cache_count': (
                    text_count + tts_count + memory_count
                )
            }
