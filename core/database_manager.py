...
class DatabaseManager:
        ...
        try:
            ...
            if cursor.fetchone():
                ...
                if 'set_id' not in columns:
                    ...
                    cursor.execute(
                        "ALTER TABLE words ADD COLUMN set_id INTEGER")

                    # 获取默认词库ID
                    cursor.execute(
                        "SELECT id FROM word_sets WHERE name='默认词库'")
                    default_set_id = cursor.fetchone()[0]

                    # 更新所有现有单词的set_id为默认词库
                    cursor.execute(
                        "UPDATE words SET set_id = ?", (default_set_id,))
                ...
                for column_name, column_type in columns_to_add:
                    if column_name not in columns:
                        ...
                        cursor.execute(
                            f"ALTER TABLE words ADD COLUMN {column_name} {column_type}")
            ...
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_proficiency ON words(proficiency)')
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_set_id ON words(set_id)')
            cursor.execute(
            ...
            # 创建日期索引
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_practice_date ON progress(practice_date)')
    ...
    def _create_default_word_set(self, conn=None):
        ...
        try:
            ...
            # 更新现有单词的set_id为默认词库
            cursor.execute(
                "UPDATE words SET set_id = ? WHERE set_id IS NULL", (default_set_id,))

            # 更新词库单词数
            cursor.execute(
                "SELECT COUNT(*) FROM words WHERE set_id = ?", (default_set_id,))
    ...
    def _update_all_word_counts(self, conn=None):
        ...
        try:
            ...
            for set_id, in sets:
                # 统计每个词库的单词数量
                cursor.execute(
                    "SELECT COUNT(*) FROM words WHERE set_id = ?", (set_id,))
                count = cursor.fetchone()[0]

                # 更新词库的单词数量
                cursor.execute(
                    "UPDATE word_sets SET word_count = ? WHERE id = ?", (count, set_id))
        ...
        def write_worker():
            while True:
                ...
                with self._write_lock:
                    ...
                    if (self._write_queue and (now -
                                               self._last_write_time).total_seconds() > 10):
    ...
    def add_word_to_set(
        ...
        try:
            ...
            # 检查单词是否已存在于当前词库中
            existing_words = self.execute_read(
                "SELECT id FROM words WHERE set_id = ? AND word = ?", (set_id, word))
            if existing_words:
            ...
            insert_params = (
                ...
                tag,
                example_translation)
            word_id = self.execute_write(
                insert_query, insert_params, immediate=True)
    ...
    def delete_word(self, word_id):
        ...
        try:
            ...
            # 更新词库单词数
            cursor.execute(
                "UPDATE word_sets SET word_count = word_count - 1 WHERE id = ?", (set_id,))
    ...
    def add_word(self, word: str, translation: str) -> bool:
        ...
        try:
            ...
            success, msg = self.add_word_to_set(
                default_set['id'], word, translation)
            return success
        except Exception as e:
            log_error(f"添加单词失败: {str(e)}")
            return False
    ...
    def get_word_translation(
            self, word: str) -> Optional[Union[str, List[Dict[str, Any]]]]:
        """获取单词翻译
    ...
    def create_dictation_tables(self):
        ...
        try:
            ...
            # 创建索引以提高查询性能
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_dictation_word ON dictation_history(word)')
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_dictation_date ON dictation_history(practice_date)')
    ...
    def remove_word(self, word: str) -> bool:
        ...
        try:
            ...
            # 删除听写历史记录
            cursor.execute(
                "DELETE FROM dictation_history WHERE word = ?", (word,))
    ...
    def update_word_translation(self, word: str, translation: str) -> bool:
        ...
        try:
            ...
            if results and results[0]['count'] > 0:
                log_info(
                    f"更新单词成功: {word} -> translation={translation}, tag={tag_value}")
                return True
            else:
                log_info(f"单词不存在: {word}")
                return False
        ...
        try:
            ...
            for col_name, col_type in required_columns.items():
                if col_name not in existing_columns:
                    cursor.execute(
                        f"ALTER TABLE exercise_sessions ADD COLUMN {col_name} {col_type}")
                    log_info(f"为exercise_sessions表添加列: {col_name}")

            # 创建索引
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_session_type ON exercise_sessions(exercise_type)')
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_session_start ON exercise_sessions(start_time)')
