...
class ForgettingCurve:
    ...
    def calculate_weight(
            self,
            mastery_score: float,
            last_reviewed: Optional[datetime] = None) -> float:
        """
        根据掌握度和最后复习时间计算单词权重
        ...
        # 应用遗忘曲线
        # 公式: weight = base_weight * (1 - mastery_score * mastery_factor) *
        #       (1 + time_interval * forget_rate)
        weight = base_weight * (1 - mastery_score * self.mastery_factor) * \
            (1 + time_interval * self.forget_rate)
...
class WordSelector:
    ...
    def select_words(
            self,
            batch_size: int = 10,
            set_id: Optional[int] = None) -> List[Dict]:
        """
        从指定词库选择单词批次
...
class LearningProgress:
    ...
    def update_word_progress(self, word: str, was_correct: bool):
        ...
        progress['mastery_score'] = curve.update_mastery_score(
            current_score, was_correct)
...
class LearningScheduler:
    ...
    def __init__(self, word_selector: WordSelector,
    ...
    def get_next_learning_batch(
        ...
        for word in words:
            ...
            if progress['last_learned']:
                word['last_learned'] = datetime.fromisoformat(
                    progress['last_learned'])
            else:
...
class LearningManager:
    ...
    def __init__(self, word_manager, audio_player):
        ...
        self.scheduler = LearningScheduler(
            self.word_selector, self.progress_manager)
    ...
    def get_batch(self, batch_size: int = 10) -> List[Dict]:
        ...
        try:
            # 使用调度器获取学习批次
            self.current_batch = self.scheduler.get_next_learning_batch(
                batch_size)
    ...
    def _update_daily_learning(self, finished=False):
        ...
        """
        self.log_info(
            "_update_daily_learning方法已废弃，"
            "使用LearningProgress.update_daily_progress替代")
    ...
    def _apply_daily_decay(self):
        ...
        """
        self.log_info(
            "_apply_daily_decay方法已废弃，"
            "使用ForgettingCurve自动计算权重")
    ...
    def load_daily_progress(self) -> bool:
        ...
        try:
            ...
            if (today_progress['current_batch'] and not
                    today_progress['completed']):
    ...
    def adjust_batch_size(
        ...
        try:
            ...
            if (not self.current_batch
                    or self.current_index >= current_batch_size - 1):
            ...
            if new_batch_size > current_batch_size:
                ...
                # 获取新单词，确保不与当前批次重复
                if set_id:
                    all_words = self.word_manager.get_words_by_set_id(set_id)
                else:
                    all_words = self.word_manager.get_words_from_active_set()
                # 获取当前批次的所有单词文本
                current_texts = {word['word'] for word in self.current_batch}

                # 过滤掉当前批次已有的单词
                available_words = []
                for word in all_words:
                    if word['word'] not in current_texts:
                ...
                self.review_count = min(
                    self.review_count,
                    new_batch_size - self.mastered_count)
