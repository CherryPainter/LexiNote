...
class DictationManager:
        ...
        try:
            ...
            if os.path.exists('data/word_dict.json'):
                ...
                try:
                    ...
                    # 批量插入单词到默认词库
                    default_set_id = self.db_manager.get_word_set_by_name(
                        '默认词库')
                    if default_set_id:
                        default_set_id = default_set_id['id']
                        word_data = [(default_set_id, word, translation)
    ...
    def _init_settings(self):
        ...
        try:
            ...
            if self.settings_manager:
                # 从设置管理器获取设置
                self.settings['auto_play_pronunciation'] = self.settings_manager.get_setting(
                    'dictation_auto_play_pronunciation', True)
                self.settings['show_example_sentence'] = self.settings_manager.get_setting(
                    'dictation_show_example', True)
                self.settings['enable_auto_mode'] = self.settings_manager.get_setting(
                    'dictation_auto_mode', False)
                self.settings['auto_mode_delay'] = self.settings_manager.get_setting(
                    'dictation_auto_delay', 3)
                self.settings['show_phonetic'] = self.settings_manager.get_setting(
                    'dictation_show_phonetic', True)
                self.settings['pronunciation_speed'] = self.settings_manager.get_setting(
                    'dictation_pronunciation_speed', 1.0)
                self.settings['difficulty_level'] = self.settings_manager.get_setting(
                    'dictation_difficulty', 'medium')
                self.settings['auto_play'] = self.settings_manager.get_setting(
                    'dictation_auto_play', True)  # 保持向后兼容
                self.settings['play_interval'] = self.settings_manager.get_setting(
                    'dictation_play_interval', 3000)  # 保持向后兼容
                self.settings['daily_target'] = self.settings_manager.get_setting(
                    'dictation_daily_target', 20)
                self.settings['review_frequency'] = self.settings_manager.get_setting(
                    'dictation_review_frequency', 3)
        except Exception as e:
    ...
    def _init_database(self):
        ...
        try:
            ...
            conn = self.db_manager.execute_read(
                "PRAGMA table_info(dictation_history)")
            ...
            if 'difficulty' not in columns:
                self.db_manager.execute_write(
                    "ALTER TABLE exercise_sessions ADD COLUMN difficulty TEXT")
    ...
    def start_session(
        ...
        try:
            ...
            # 选择单词
            if mode == 'queue':
                self.current_queue = self.build_queue(
                    source, batch_size, difficulty=difficulty)
    ...
    def end_session(self):
        ...
        try:
            ...
            correct_words = sum(
                1 for result in self.session_results if result['is_correct'])
            ...
            log_info(
                f"会话结束: ID={
                    self.current_session['id']}, 时长={duration}秒, 单词数={total_words}, 正确率={
                    accuracy:.2f}")
    ...
    def get_stats(self, days: int = 7):
        ...
        try:
            # 获取当前激活词库ID
            active_set_id = self.word_manager.active_word_set_id if hasattr(
            ...
            SELECT DATE(practice_date) as date,
               ...
               ORDER BY date
            """
            daily_stats = self.db_manager.execute_read(
                daily_stats_query, params)
            ...
            SELECT id, mode, source, start_time, duration, total_words, accuracy
               ...
               LIMIT 10
            """
            session_stats = self.db_manager.execute_read(
                session_stats_query, [start_date])
            ...
            today_practices = self.db_manager.execute_read(
                today_query, [today])[0]['count']
            daily_target = self.settings['daily_target']
            target_progress = min(
                100, (today_practices / daily_target) * 100) if daily_target > 0 else 0
    ...
    def _fetch_and_save_example_async(self, word):
        ...
        def _fetch_task():
            try:
                ...
                if hasattr(
                        self.word_manager,
                        'ai_available') and self.word_manager.ai_available:
                    if hasattr(
                            self.word_manager,
                            'ai_manager') and hasattr(
                            self.word_manager.ai_manager,
                            'get_example_sentence'):
                        # 调用AI获取例句
                        example = self.word_manager.ai_manager.get_example_sentence(
                            word)
                        if example:
                            # 保存到数据库
            ...
            if source == "today":
                ...
                if difficulty == "easy":
                    ...
                    if len(words) < limit:
                        ...
                        LIMIT ?
                        """
                        wrong_results = self.db_manager.execute_read(
                            wrong_query, [remaining])
                    ...
                    if old_limit > 0:
                        ...
                        if words:
                            subquery += f" OR word IN ({
                                ','.join(
                                    ['?'] * len(words))})"
                            words_params = words.copy()
                        else:
                            words_params = []
    ...
    def _get_today_learned_words(self, with_example=False):
        ...
        try:
            ...
            # 检查缓存是否有效（缓存有效期为60秒）
            now = datetime.now()
            if (self._today_words_cache_time
                    and (now - self._today_words_cache_time).total_seconds() < 60
                    and cache_key in self._today_words_cache):
                log_debug(
                    f"使用缓存的今日学习单词: {len(self._today_words_cache[cache_key])}个")
    ...
    def process_result(self, word, user_input, is_correct, time_spent=0):
        ...
        try:
            # 记录练习结果
            log_info(
                f"处理听写结果: {word} - {'正确' if is_correct else '错误'}，用时: {time_spent}秒")
            ...
            self.record_dictation_result(
                word, user_input, is_correct, similarity)

            # 更新单词权重（复用WordManager的逻辑）
            if hasattr(self.word_manager, 'update_word_weight'):
                try:
                    self.word_manager.update_word_weight(
                        word, is_correct, time_spent)
                except Exception as e:
            ...
            if hasattr(self.word_manager, 'update_word_familiarity'):
                try:
                    if is_correct:
                        self.word_manager.update_word_familiarity(
                            word, 0.1)  # 正确增加熟悉度
                    else:
                        self.word_manager.update_word_familiarity(
                            word, -0.15)  # 错误降低熟悉度
            ...
            if self.db_manager:
                ...
                self.db_manager.execute_write(
                    """INSERT INTO dictation_history (word, user_input, is_correct, similarity, practice_date)
                       VALUES (?, ?, ?, ?, ?)""", (word, user_input, is_correct_int, similarity, timestamp))
                ...
                try:
                    ...
                    if wp and 'proficiency' in wp:
                        self.db_manager.update_proficiency(
                            word, wp['proficiency'])
                    else:
                        # 如果无法获取聚合数据，直接更新
                        ...
                        new_proficiency = max(
                            0.0, min(
                                1.0, current_proficiency + proficiency_change))
    ...
    async def record_dictation_result_async(
            self,
            word: str,
            user_input: str,
            is_correct: bool,
    ...
    def _update_word_progress(self, word: str, is_correct: bool):
        ...
        try:
            ...
            self.record_dictation_result(
                word, word if is_correct else "", is_correct)
        except Exception as e:
        ...
        try:
            ...
            stats = {
                ...
                'most_wrong_words': [
                    {
                        'word': row['word'],
                        'count': row['wrong_count']} for row in most_wrong_words]}
    ...
    def summarize(self, queue=None, callback=None):
        ...
        correct = sum(
            ...
            or r.get("is_correct", False)
        )
        accuracy = correct / total if total > 0 else 0
        missed = [
            r.get("word") for r in queue_records
            if r.get("result") != "correct"
        ...
        try:
            ...
            if hasattr(self, 'settings_manager') and self.settings_manager:
                ai_summary_enabled = self.settings_manager.get_setting(
                    "ai_summary_enabled", True)

            if ai_summary_enabled and hasattr(
                    self.word_manager,
                    'ai_available') and self.word_manager.ai_available:
                ...
                if queue_records:
                    valid_times = [
                        r.get(
                            'time_spent',
                            0) for r in queue_records if r.get(
                            'time_spent',
                            0) > 0]
                    if valid_times:
                ...
                if hasattr(
                    ...
                    if hasattr(
                            self.word_manager.ai_manager,
                            'advise_stream') and callback:
                        # 使用真正的流式输出API
                        suggestion = ""
                        for chunk in self.word_manager.ai_manager.advise_stream(
                                user_stats):
                        ...
                        full_suggestion = self.word_manager.ai_manager.advise(
                            user_stats)
                        # 逐字发送建议，实现真正的流式显示效果
                        ...
                        # 正常获取AI建议
                        suggestion = self.word_manager.ai_manager.advise(
                            user_stats)
        except Exception as e:
        ...
        if hasattr(self.word_manager, 'get_difficult_words'):
            try:
                difficult_words = self.word_manager.get_difficult_words(
                    limit=5)
            except Exception as e:
        ...
        try:
            ...
            if hasattr(
                ...
                if hasattr(
                        ...
                        'ai_manager') and hasattr(
                        self.word_manager.ai_manager,
                        'analyze_dictation_results'):
                    # 调用AI分析方法
                    analysis = self.word_manager.ai_manager.analyze_dictation_results(
                        results_json)
            ...
            # 生成简单的本地分析
            correct_count = sum(
                1 for item in self.completed_words if item['is_correct'])
            total_count = len(self.completed_words)
            accuracy = correct_count / total_count if total_count > 0 else 0

            analysis = f"听写完成！正确率: {
                accuracy:.1%} ({correct_count}/{total_count})"
            if not all(item['is_correct'] for item in self.completed_words):
                wrong_words = [
                    item['word'] for item in self.completed_words if not item['is_correct']]
    ...
    def next_in_queue(self):
        ...
        """
        # 确保队列存在且索引有效
        if not self.current_queue or self.current_queue_index >= len(
                self.current_queue):
            return None
    ...
    def has_next_in_queue(self):
        ...
        Returns:
            是否还有下一个单词
        """
        return self.current_queue and self.current_queue_index < len(
            self.current_queue)
