...
## v2.1.0 - 2025-10-25
### 修复
- 修复了设置模块UI阻塞问题（使用after()延迟加载AI模型，避免界面卡顿）
- 修复了翻译页面和复习页面自动切换失效问题（将get_auto_mode改为正确的get_setting方法）
- 修复了翻译判定模式失效问题（添加translation_mode参数，实现三种判断策略：ai_first/local_first/local_only）
- 修复了翻译练习中判断方式显示错误问题（根据实际设置显示正确的判断方式文本）

## v2.0.0 - 2025-11-18
### 功能升级
- **翻译系统升级**：将单词翻译从简单字符串升级为支持多词性多义项的结构化数据
- **兼容性处理**：实现了同时支持旧格式(string)和新格式(array)的翻译处理逻辑
- **展示模块更新**：修改了`get_translation`方法，添加`format_output`参数支持多种输出格式
- **翻译判断模块更新**：更新`check_translation`方法，支持从多词性多义项结构中提取候选翻译
- **AI补全模块更新**：要求AI返回结构化的中文释义，并转换为多词性多义项结构
- **数据库管理更新**：扩展支持新的翻译结构
- **旧数据迁移**：实现旧格式翻译数据的自动转换，支持将现有字符串格式翻译拆分为结构化数据
...
- 重构窗口初始化和图标设置逻辑：将窗口设置移至main_window.py并添加PyInstaller打包支持
- 升级版本号到v1.11.8
...
- 补充了settings和ai_cache表的信息到表关系图中

## v1.11.2 - 2025-11-17
### Enhanced
- **日志执行逻辑优化**：修改了页面切换日志顺序，确保日志按照实际执行顺序记录
- **AIManager单例实现**：避免重复初始化AI管理器，减少资源消耗
- **模块冗余初始化修复**：各模块接受外部WordManager实例并共享主窗口实例，提高资源利用率
- **AI连接测试逻辑优化**：实现连接状态共享，避免重复测试AI连接
- **修复重复日志问题**：解决了"AI服务连接测试成功"重复出现的问题，确保日志记录准确
