...
- **完形填空页面UI阻塞问题**：将 ClozeTestModule 初始化移到后台线程，避免首次打开页面时因AI连接测试导致的UI阻塞。
- **多余的AI初始化和测试**：优化了 AIService 的初始化流程，避免在已有 WordManager 实例的情况下重复进行AI连接测试。
...
- 修复设置页面重置功能中不存在的_load_ai_models方法调用错误
